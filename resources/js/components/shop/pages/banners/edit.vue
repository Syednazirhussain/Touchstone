	...
	export default {
		...
		methods: {
			async getBanner (id) {
	    		...
	    		let urlArr = _.split(window.touchStone.routes.shop_admin_banner_edit, '/', 3)
			...
			async update () {
	    		...
	    		let urlArr = _.split(window.touchStone.routes.shop_admin_banner_update, '/', 3)
				let url = _.join(urlArr, '/')+ '/' +this.$route.params.id+ '/update'

				await axios.put(url, this.banner).then(response => {
