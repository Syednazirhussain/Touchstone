						...
						<div class="col-md-12" v-if="isLoaded">
						    <div class="box box-primary">
								<div class="box-header with-border">
								    <h3 class="box-title" v-if="isFound">{{ getText(faq.product.title) }}</h3>
								    <div class="box-tools">
	...
	export default {
		...
		methods: {
			...
			async getFaq (id) {
				...
				let url = _.join(urlArr, '/')+ '/' +this.product_id+ '/faq/' +this.id+ '/edit'
				...
				await axios.get(url).then(response => {
					...
					this.faq.answer 	= JSON.parse(this.faq.answer)
					this.faq.question 	= JSON.parse(this.faq.question)
			...
			async update () {
	    		...
	    		let urlArr = _.split(window.touchStone.routes.shop_admin_product_faq_update, '/', 3)
				let url = _.join(urlArr, '/')+ '/' +this.product_id+ '/faq/' +this.id+ '/update'

				await axios.put(url, this.faq).then(response => {
