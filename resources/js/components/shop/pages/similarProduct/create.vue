									    		...
									    		<div class="col-md-12 form-group">
									    			...
									    			<div class="col-md-4 pull-left">
									    				<select class="form-control" v-model="similarProduct.similar_product_id">
										    					<option :value="product.id" v-for="(product, index) in products" :key="index" :selected="index == 1">
	...
	export default {
		...
		methods: {
			...
			async store () {
	    		...
	    		let urlArr = _.split(window.touchStone.routes.shop_admin_product_product_similar_store, '/', 3)
				let url = _.join(urlArr, '/')+ '/' +this.product_id+ '/similar/store'

				this.similarProduct.product_id = this.product_id

				await axios.post(url, this.similarProduct).then(response => {
